...
class PlotlyVisualize(MotherVisualization):
    ...
    def investigate_resulting_load_profile(self):
        ...
        fig.add_trace(go.Histogram(fig_sems["source"][0], opacity=0.3, name="SEMS load"),
                      col=1, row=1)
        fig.update_traces(col=1, row=1, marker_color="red")

        fig.add_trace(go.Histogram(fig_ref["source"][0], opacity=0.3, name="Reference load"),
                      col=1, row=1)
        ...
        path_to_image_folder = r"C:/Users/mascherbauer/PycharmProjects/NewTrends/Prosumager/_Refactor/project/PhilippTest/Figures/" + image_name
