...
class OptOperationModel(OperationModel):
    ...
    def setup_abstract_model(self):
        ...
        m.Q_HeatingTank = pyo.Var(
            m.t, within=pyo.NonNegativeReals
        )  # energy in the tank
        ...
        def calc_UseOfGrid(m, t):
            return (
                m.Grid[t]
                == m.Grid2Load[t] + m.Grid2Bat[t] + m.Grid2EV[t] * m.EVAtHomeProfile[t]
            )
        ...
        m.SumOfLoads_with_cooling_rule = pyo.Constraint(
            m.t, rule=calc_SumOfLoads_with_cooling
        )
        ...
        m.SumOfLoads_without_cooling_rule = pyo.Constraint(
            m.t, rule=calc_SumOfLoads_without_cooling
        )
        ...
        def calc_BatCharge(m, t):
            return (
                ...
                == m.PV2Bat[t]
                + m.Grid2Bat[t]
                + m.EV2Bat[t] * m.EVAtHomeProfile[t] * self.EVOptionV2B
            )
        ...
        def calc_EVCharge(m, t):
            return (
                m.EVCharge[t]
                == m.PV2EV[t] * m.EVAtHomeProfile[t]
                + m.Grid2EV[t] * m.EVAtHomeProfile[t]
        ...
        def calc_BatDischarge(m, t):
            if m.t[t] == 1:
                ...
                return (
                    m.BatDischarge[t]
                    == m.Bat2Load[t] + m.Bat2EV[t] * m.EVAtHomeProfile[t]
                )
        ...
        def calc_BatSoC(m, t):
            if t == 1:
                ...
                return m.BatSoC[t] == m.BatSoC[t - 1] + m.BatCharge[
                    t
                ] * self.ChargeEfficiency - m.BatDischarge[t] * (
                    1 + (1 - self.DischargeEfficiency)
                )
        ...
        def calc_EVSoC(m, t):
            if t == 1:
                ...
                return m.EVSoC[t] == m.EVSoC[t - 1] + m.EVCharge[
                    t
                ] * self.EVChargeEfficiency * m.EVAtHomeProfile[t] - m.EVDischarge[
                    t
                ] * (
                    1 + (1 - self.EVDischargeEfficiency)
                )
        ...
        def tank_energy_heating(m, t):
            if t == 1:
                return (
                    m.Q_HeatingTank[t]
                    == self.CPWater
                    * self.M_WaterTank_heating
                    * (273.15 + self.T_TankStart_heating)
                    - m.Q_HeatingTank_out[t]
                )
            else:
                return m.Q_HeatingTank[t] == m.Q_HeatingTank[
                    t - 1
                ] - m.Q_HeatingTank_out[t] + m.Q_HeatingTank_in[
                    t
        ...
        m.E_RoomCooling_with_cooling_rule = pyo.Constraint(
            m.t, rule=calc_E_RoomCooling_with_cooling
        )
        ...
        def thermal_mass_temperature_rc(m, t):
            ...
            PHI_mtot = (
                ...
                * (
                    ...
                    * (
                        (
                            (self.PHI_ia + m.Q_RoomHeating[t] - m.Q_RoomCooling[t])
                            / self.Hve
                        )
                        + T_sup
                    )
            ...
            return m.T_BuildingMass[t] == (
                Tm_start * ((self.Cm / 3600) - 0.5 * (self.Htr_3 + self.Htr_em))
                + PHI_mtot
            ) / ((self.Cm / 3600) + 0.5 * (self.Htr_3 + self.Htr_em))

        m.thermal_mass_temperature_rule = pyo.Constraint(
            m.t, rule=thermal_mass_temperature_rc
        )
    ...
    def params_dict(self) -> dict:
        pyomo_dict = {
            None: {
                ...
                "SpaceHeatingHourlyCOP_tank": self.create_dict(
                    self.SpaceHeatingHourlyCOP_tank
                ),
                # hot water
    ...
    def config_instance(self, instance):
        ...
        for t in range(1, 8761):
            instance.Q_HeatingElement[t].setub(
                self.scenario.boiler.heating_element_power
            )
            instance.T_Room[t].setlb(
                self.scenario.behavior.target_temperature_array_min[t - 1]
            )
        ...
        if self.scenario.space_cooling_technology.power == 0:
            no_cooling_target_temperature_max = (
                self.generate_maximum_target_indoor_temperature_no_cooling(
            ...
            for t in range(1, 8761):
                ...
                instance.Q_RoomCooling[t].setub(
                    self.scenario.space_cooling_technology.power
                )
                instance.T_Room[t].setub(
                    self.scenario.behavior.target_temperature_array_max[t - 1]
                )
        ...
        if self.scenario.space_heating_tank.size == 0:
            ...
            for t in range(1, 8761):
                ...
                instance.Q_HeatingTank[t].setlb(
                    self.CPWater
                    * self.scenario.space_heating_tank.size
        ...
        if self.scenario.hot_water_tank.size == 0:
            ...
            for t in range(1, 8761):
                ...
                instance.Q_DHWTank[t].setlb(
                    self.CPWater
                    * self.scenario.hot_water_tank.size
        ...
        if self.scenario.battery.capacity == 0:
            ...
            for t in range(1, 8761):
                ...
                instance.BatDischarge[t].setub(
                    self.scenario.battery.discharge_power_max
                )
    ...
    def run_fuel_boiler_opt(self):
        ...
        solved_instance.__dict__["T_outside"].store_values(
            self.create_dict(outside_temperature)
        )
        solved_instance.__dict__["Q_Solar"].store_values(self.create_dict(q_solar))
        solved_instance.__dict__["Q_RoomHeating"].set_values(
            self.create_dict(heating_demand)
        )
        solved_instance.__dict__["Q_RoomCooling"].set_values(
            self.create_dict(cooling_demand)
        )
        solved_instance.__dict__["E_RoomCooling"].set_values(
            self.create_dict(cooling_demand / self.CoolingCOP)
        )
        solved_instance.__dict__["T_Room"].set_values(
            self.create_dict(room_temperature)
        )
        solved_instance.__dict__["T_BuildingMass"].set_values(
            self.create_dict(building_mass_temperature)
        )
        solved_instance.__dict__["HotWaterProfile"].store_values(
            self.create_dict(hot_water_demand)
        )
        solved_instance.__dict__["total_operation_cost_rule"] += (
            heating_cost + cooling_cost + hot_water_cost
        )
