...
class OperationAnalyzer:
    ...
    def plot_electricity_balance(self, scenario_id: int, model: str, start_hour: int = None, end_hour: int = None):
        ...
        self.plotter.bar_plot(values_dict, f"ElectricityBalance_S{scenario_id}_H{start_hour}To{end_hour}_{model}",
    ...
    def plot_operation_energy_cost_change_curve(self, component_changes: List[Tuple[str, int, int]]):
        ...
        for component_change_info in component_changes:
            ...
            benchmark_id = component_change_info[1]
            state_id = component_change_info[2]
            filter_dict = {col_component_id: 0, "ID_Benchmark": benchmark_id, "ID_State": state_id}
        ...
        self.plotter.step_figure(values_dict=values_dict, fig_name=f"OperationEnergyCostChange",
                                 x_label="Scenarios", y_label="Energy Saving Benefit (€/a)", y_lim=(-100, 2000))

    def get_operation_energy_cost_change(self, building_dict: dict, component_change_info: Tuple[str, int, int]):
        ...
        energy_cost_change = kit.filter_df2s(operation_energy_cost_change, filter_dict=building_dict)["CostChange"]
        return energy_cost_change

    @staticmethod
    def update_component_changes(building_dict: dict, component_changes: List[Tuple[str, int, int]]):
        ...
        updated_component_changes = [info for info in component_changes if not implementable(building_dict, info)]
        return updated_component_changes

    @staticmethod
    def implement_building_component_change(building_dict: dict, component_change_info: Tuple[str, int, int]):
        col_component_id = component_change_info[0]
        benchmark_id = component_change_info[1]
        state_id = component_change_info[2]
        logger.info(f'ComponentChange: {col_component_id} = {benchmark_id} -> {col_component_id} = {state_id}')
        assert building_dict[col_component_id] == benchmark_id, "ComponentChange cannot be implemented."
        building_dict[col_component_id] = state_id
        return building_dict

    def get_building_pathway(self, scenario_id: int, component_changes: List[Tuple[str, int, int]]):
        df = self.db.read_dataframe(InterfaceTable.OperationEnergyCost.value)
        df = df.drop(["ID_OperationScenario", "TotalCost"], axis=1)
        building_dict = kit.filter_df2s(df, filter_dict={"ID_Scenario": scenario_id}).to_dict()
        del building_dict["ID_Scenario"]
        logger.info(f'Initial building: {building_dict}.')

        while len(component_changes) > 0:
            component_changes = self.update_component_changes(building_dict, component_changes)
            component_change_benefit = []
            for component_change_info in component_changes:
                benefit = self.get_operation_energy_cost_change(building_dict, component_change_info)
                component_change_benefit.append([component_change_info, benefit])
            component_change_benefit = sorted(component_change_benefit, key=lambda x: x[1], reverse=True)
