...
class OperationScenario:
    ...
    def setup_behavior_target_temperature(self, behavior: pd.DataFrame):
        ...
        self.behavior.target_temperature_array_max, self.behavior.target_temperature_array_min = \
            self.gen_target_temperature_range_array(behavior[column].to_numpy(),
    ...
    def gen_profile_with_annual_amount(annual_amount: float, shape_array: np.ndarray) -> np.ndarray:
        profile = np.zeros(len(shape_array), )
    ...
    def setup_behavior_vehicle(self, behavior: pd.DataFrame):
        column_at_home = f'vehicle_at_home_profile_{self.behavior.id_vehicle_at_home_profile}'
        column_distance = f'vehicle_distance_profile_{self.behavior.id_vehicle_distance_profile}'
        self.behavior.vehicle_at_home = behavior[column_at_home].to_numpy() + 0.0000001  # pyomo Problem with 0
