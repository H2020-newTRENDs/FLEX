...
def filter_only_electrified_buildings(df: pd.DataFrame) -> pd.DataFrame:
    # remove buildings without any heat pumps
    df_heat_pumps = df[
        (df["number_buildings_heat_pump_air"] > 0) |
        (df["number_buildings_heat_pump_ground"] > 0) |
        (df["number_buildings_electricity"] > 0)
        ].reset_index(drop=True)
    df_heat_pumps.loc[:, "index"] = df_heat_pumps.index + 1
...
def reduce_number_of_buildings(df: pd.DataFrame) -> pd.DataFrame:
    ...
    electric_numbers = df["number_buildings_electricity"].sum()
    ...
    heating_systems = ["number_buildings_heat_pump_air",
                       "number_buildings_heat_pump_ground",
                       "number_buildings_electricity"]
    big_df = pd.DataFrame()
    for index, group in grouped_df:
        ...
        ground_hp = create_representative_building(group,
                                                   ...
                                                   merging_names=merging_names,
                                                   adding_names=adding_names)
        direct_elec = create_representative_building(group,
                                                     heating_system_name="number_buildings_electricity",
                                                     merging_names=merging_names,
                                                     adding_names=adding_names)
        ...
        direct_elec[["number_buildings_heat_pump_air", "number_buildings_heat_pump_ground"]] = 0

        new_group = pd.concat([air_hp, ground_hp, direct_elec]).reset_index(drop=True)
        # sometimes the number of heating systems is zero: drop these columns

        # now we have 3 representative buildings for the group but all these buildings have the total number of PV
    ...
    assert round(return_df["number_buildings_heat_pump_air"].sum()) == round(air_hp_numbers) and \
           ...
           round(return_df["number_buildings_electricity"].sum()) == round(electric_numbers), \
...
def create_operation_weather_file(country: str, big_weather_df: pd.DataFrame) -> pd.DataFrame:
    country_abbreviation = get_country_correction_code()[country]
    single_country_df = big_weather_df.loc[big_weather_df.loc[:, "region"] == country_abbreviation, :]
    return single_country_df.reset_index(drop=True)
...
def load_reduced_building_parquet_file(year: int, country: str, source_path: Path) -> pd.DataFrame:
    path = source_path / country
    df = load_invert_data(path_to_file=path / f"{year}.parquet.gzip")
    # turn int values into int:
    int_columns = ["construction_period_start", "construction_period_end", "building_categories_index",
                   "number_of_floors"]
    df[int_columns] = df[int_columns].astype(int)
    # replace nan with 0:
    df = df.fillna(0)
    df_heat_pumps = filter_only_electrified_buildings(df)  # filter out buildings without electric heating
    df_reduced = reduce_number_of_buildings(df_heat_pumps)  # reduce the number of buildings by merging similar ones
...
def create_input_excels(year: int,
    ...
    weather_scenario_table = create_operation_weather_file(country, weather_df)
...
def main(country_list: list, years: list, minimum_number_buildings: int, project_prefix: str, source_path: Path):
    # original weather file:
    logger.info("loading weather file...")
    original_file = Path(__file__).parent / "data" / "input_operation" / "OperationScenario_RegionWeather.xlsx"
    original_df = pd.read_excel(io=original_file, engine="openpyxl", sheet_name=0)
    logger.info("finished loading weather file")
    ...
    if answer.lower() == "y":
        ...
        input_list = [(year, country, original_df, project_prefix, source_path, force_copy)
...
if __name__ == "__main__":
    ...
    project_prefix = f"D5.4_low"
    source_path = Path(__file__).parent / "projects" / f"NewTrends_{project_prefix}"
