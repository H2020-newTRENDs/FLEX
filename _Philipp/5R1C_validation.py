...
def showResults_noDR(Q_Heating_noDR, Q_Cooling_noDR, T_Room_noDR, T_thermalMass_noDR, plot_EPlus=False):
    if plot_EPlus == True:
        B1_EPlus = pd.read_csv(
            "C:\\Users\\mascherbauer\\PycharmProjects\\NewTrends\\Prosumager\\_Philipp\\inputdata\\Building1_EPlus.csv",
            sep=";")
    ...
    for i in range(3):
        ...
        plt.plot(x_achse, T_thermalMass_noDR[:, i], label="thermal mass temperature " + str(i + 1), color=colors2[i],
...
def showResults_Sprungantwort(Q_noDR, T_Room_noDR, T_thermalMass_noDR, heatORcool):
    ...
    ax2 = ax1.twinx()
    for i in range(3):
        ax1.plot(x_achse, T_thermalMass_noDR[:, i], label="thermal mass temperature " + str(i + 1), color=colors2[i],
                 alpha=0.8, linewidth=width, linestyle=":")
        ax2.plot(x_achse, Q_noDR[:, i] / 1_000, label=heatORcool + " power " + str(i + 1), color=colors[i], alpha=0.8,
    ...
    plt.tight_layout()
    fig3.savefig(
        "C:\\Users\\mascherbauer\\PycharmProjects\\NewTrends\\Myfigs\\Paper_figs\\Sprungantwort_" + heatORcool + ".svg")
    plt.show()
...
def calculate_LoadShiftPotential(Buildings, hours_of_preheating, hours_of_shifting, T_outside,
    ...
    ax2.hlines(T_thermalMass_noDR_constant[0], xmin=0, xmax=hours_of_preheating + hours_of_shifting - 1, color="purple",
...
def compare_solar_radation():
    ...
    ax2.legend()
    fig.savefig(
        "C:\\Users\\mascherbauer\\PycharmProjects\\NewTrends\\Myfigs\\Paper_figs\\SolarRadiationVergleichEPlus.svg")
    plt.show()
