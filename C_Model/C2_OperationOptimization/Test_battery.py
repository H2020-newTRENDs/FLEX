...
#fixed profiles
load_profile = create_dict([0,0,0,0, 0,1,3,1, 1,1,2,3, 2,1,1,1, 1,2,3,2, 1,0,0,0]*int(hours_of_simulation/24))
...
def BatChargeandDischarge(m,t):
    if t == 1:
        ...
        return m.sfl[t] == m.sfl[t-1] + m.bat_charge[t] - m.bat_discharge[t]
...
#    plt.plot(x_axis, bat_discharge, label='Discharge', color='red', linestyle='dotted', linewidth =0.5)
    plt.plot(x_axis, pv_profile, label='PV_Profile', color='orange', linestyle='solid')
    plt.plot(x_axis, pv_grid, label='PV_grid', color='green', linestyle='solid')
