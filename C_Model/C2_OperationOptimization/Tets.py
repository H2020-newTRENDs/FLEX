...
class OperationOptimization:
    ...
    def run_Optimization(self, household_id, environment_id):
        ...
        #########################################################################################
        # just test for EV and Bat - later from DB

        if Household.ElectricVehicle.BatterySize == 0:
            CarAtHome = create_dict([0] * HoursOfSimulation)
        else:
            CarAtHome = self.CarAtHome.CarAtHomeHours.to_numpy()
        ...
        m.CarStatus = pyo.Param(m.t, initialize=create_dict(CarAtHome))
        ...
        m.BatSoC = pyo.Var(m.t, within=pyo.NonNegativeReals, bounds=(0, 10))
        ...
        def minimize_cost(m):
            rule = sum(m.Grid[t] * m.ElectricityPrice[t] - m.Feedin[t] * 0.0792 for t in m.t)
            return rule
        ...
        def calc_EVSoC(m, t):
            if t == 1:
                ...
                return m.EVSoC[t] == m.EVSoC[t - 1] + (m.EVCharge[t] * m.CarStatus[t] * 0.95) - (
                    ...
                    t] * 1.05) - (1 * (1 - m.CarStatus[t])) - (
                               Household.ElectricVehicle.BatterySize * 0.02 / 30 / 24)  # self discharge
...
def show_results(instance, HoursOfSimulation, ListOfDynamicCOP, M_WaterTank, CWater, colors):
    ...
    # battery
    BatSoC = np.array(list(instance.BatSoC.extract_values().values())[starttime: endtime])
    ...
    Bat2Load = np.array(list(instance.Bat2Load.extract_values().values())[starttime: endtime])
    ...
    # EV
    EVSoC = np.array(list(instance.EVSoC.extract_values().values())[starttime: endtime])
    EVCharge = np.array(list(instance.EVCharge.extract_values().values())[starttime: endtime])
    EVDischarge = np.array(list(instance.EVDischarge.extract_values().values())[starttime: endtime])
    ...
    CarStatus = np.array(list(instance.CarStatus.extract_values().values())[starttime: endtime])
    ...
    PV2Bat = np.array(list(instance.PV2Bat.extract_values().values())[starttime: endtime])
    ...
    ax1.bar(x_achse, -EV2Load, bottom=-PV2Load + -Bat2Load + -EV2Load, color='grey', alpha=0.5)
    ...
    #             ElectricityCooling),
    #         label="BatCharge")
    # plt.bar(x_achse, Feedin, color="green",
    #         bottom=np.array(LoadProfile) + np.array(HotWater) + np.array(ElectricityDemandHeatPump) + np.array(
    ...
    # plt.plot(x_achse, Grid, linewidth=1, label='GridCover',
