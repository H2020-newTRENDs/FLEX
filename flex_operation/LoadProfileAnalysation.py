...
from flex_operation.constants import OperationTable, OperationScenarioComponent, OperationResultVar
...
class LoadProfileAnalyser:
    ...
    def mean_absolute_change(self, profile: np.array) -> float:
        """
        The MAC captures both the magnitude of changes (through the absolute values) and the frequency of changes
        (since it's an average over all intervals). A higher MAC indicates either larger changes, more frequent
        changes, or both, in the load profile.
        :param profile: profile as 1D numpy array
        :return: return the MAC as float
        """
        if len(profile) < 2:
            raise ValueError("The load profile should have at least two data points.")
            # Calculate absolute changes between consecutive data points
        absolute_changes = [abs(profile[i + 1] - profile[i]) for i in range(len(profile) - 1)]
        # Calculate and return the mean of the absolute changes
        return sum(absolute_changes) / (len(profile) - 1)
    ...
    def grab_max_q_room_heating_plus_hot_water_per_building(self):
        """This function is for a quick analysis for Songmin. Get the max value of Q_RoomHeating+HoWater from the
        hourly values. Save it to df with columns: country, Building type (includes SFH, MFH, age, Af and norm heat
        demand."""
        print(self.country)
        building_table = self.db.read_dataframe(table_name=OperationScenarioComponent.Building.table_name)
        building_ids = building_table["ID_Building"].values.tolist()
        from dash_visualization.data_extraction import create_scenario_selection_table_with_numbers
        selection_table = data_extraction.create_scenario_selection_table(
            project_name=self.project_name
        )

        selected_df = selection_table.query(f"ID_Battery == 0 & "
                                            f"ID_Building in @building_ids &"
                                            f"ID_EnergyPrice == 1 &"
                                            f"ID_HeatingElement == 0 &"
                                            f"ID_HotWaterTank == 0 &"
                                            f"ID_SpaceHeatingTank == 0 &"
                                            f"ID_Vehicle == 0")
        used_building_ids = []
        songmin_df = pd.DataFrame()
        for i, row in selected_df.iterrows():
            building_id = int(row["ID_Building"])
            if building_id in used_building_ids:
                continue  # skip double buidling IDs because the heating system has no influence on the results that he needs
            used_building_ids.append(building_id)

            scenario_id = row["ID_Scenario"]
            reference_load = self.db.read_parquet(table_name=OperationTable.ResultRefHour,
                                                  scenario_ID=scenario_id,
                                                  ).loc[:, ["HotWaterProfile",
                                                            "Q_RoomHeating"]].to_numpy().sum(axis=1)
            sems_load = self.db.read_parquet(table_name=OperationTable.ResultOptHour,
                                             scenario_ID=scenario_id,
                                             ).loc[:, ["HotWaterProfile",
                                                       "Q_RoomHeating"]].to_numpy().sum(axis=1)
            # calculate maximum
            ref_max = max(reference_load)
            opt_max = max(sems_load)
            # total heat + hot water demand
            total_heat_demand = reference_load.sum()

            # building information:
            building_info = building_table.loc[building_table["ID_Building"] == building_id, [
                "Af", "type", "construction_period_start", "construction_period_end"
            ]].reset_index(drop=True)
            building_info.loc[:, ["ref_max_heat (Wh)", "opt_max_heat (Wh)", "sum_heat_hot_water (Wh)"]] = [ref_max, opt_max, total_heat_demand]
            songmin_df = pd.concat([songmin_df, building_info], axis=0, ignore_index=True)
        songmin_df.loc[:, "country"] = self.country
        return songmin_df
...
if __name__ == "__main__":
    ...
    country = "AUT"
    year = 2020
    # LoadProfileAnalyser(project_name=f"D5.4_{country}_{year}", country=country,
    #                     year=year).grab_max_q_room_heating_plus_hot_water_per_building()

    big_df = pd.DataFrame()
    for country in COUNTRIES:
        df =     LoadProfileAnalyser(project_name=f"D5.4_{country}_{year}", country=country,
                        year=year).grab_max_q_room_heating_plus_hot_water_per_building()
        big_df = pd.concat([big_df, df], axis=0)

    big_df.to_csv("/home/users/pmascherbauer/projects/Philipp/PycharmProjects/projects/NewTrends_D5.4/songmin.csv", index=False, sep=";")