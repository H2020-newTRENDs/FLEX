...
class CountryResultPlots:
    ...
    def grab_EU27_mean_absolute_price_change(self):
        big_df = pd.DataFrame()
        for project in self.projects:
            try:
                LPA = LoadProfileAnalyser(project_name=project['name'],
                                          year=project['year'],
                                          country=project['country'])
                price_profiles = LPA.get_energy_price_profiles()
                MAC = {key: LPA.mean_absolute_change(value) * 1_000 for key, value in price_profiles.items()}  # cent/kWh
                df = pd.DataFrame.from_dict(MAC,
                                            orient="index",
                                            columns=[f"MAC electricity price (cent/kWh)"])
                ...
                print(f"MAC electricity price for {project['country']} {project['year']} could not be loaded.")
        big_df["MAC electricity price (cent/kWh)"] = big_df["MAC electricity price (cent/kWh)"].astype(float)
        return big_df
...
if __name__ == "__main__":
    ...
    countries = ["DEU", "AUT"]
    project_prefix = "D5.4"
    CountryResultPlots(countries=countries, years=years, project_prefix=project_prefix).grab_EU27_mean_absolute_price_change()
