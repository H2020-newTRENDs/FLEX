...
class OperationScenario:
    ...
    def setup_region_weather_and_pv_generation(self):
        df_unfiltered = self.tables.__getattribute__(OperationTable.RegionWeatherProfile)
        df = df_unfiltered.loc[df_unfiltered.loc[:, "region"] == self.region.code, :]
    ...
    def setup_energy_price(self):
        df_unfiltered = self.tables.__getattribute__(OperationTable.EnergyPriceProfile)
        # df = df_unfiltered.loc[df_unfiltered.loc[:, "region"] == self.region.code, :]
        for key, value in self.energy_price.__dict__.items():
            if key.startswith("id_") and value is not None:
                ...
                self.energy_price.__dict__[energy_carrier] = df_unfiltered[
                    energy_price_column
                ].to_numpy()
            else:
                pass
