...
class MonthConverter:
    ...
    def process_tables(self):
        ...
        for id_scenario in range(1, 4321):
            ...
            df_opt = self.db.read_dataframe(OperationTable.ResultOptHour, filter={"ID_Scenario": id_scenario})
            df_opt = self.drop_vars(df_opt)
            opt_month += self.convert_h2m(id_scenario, df_opt)

            df_ref = self.db.read_dataframe(OperationTable.ResultRefHour, filter={"ID_Scenario": id_scenario})
