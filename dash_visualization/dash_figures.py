...
def EU27_electricity_consumption_difference_country_level(app: Dash) -> html.Div:
    @app.callback(Output(ids.ELECTRICITY_CONSUMPTION_DIFFERENCE, "children"),
                  Input(ids.ALL_COUNTRIES_DROP_DOWN, "value"),
                  Input(ids.ALL_YEARS_DROP_DOWN, "value"))
    def update_Eu27_electricity_difference_consumption(countries: List[str], years: List[str]):
        year_list = [int(year) for year in years]
        big_df = pd.DataFrame()
        for country in countries:
            for year in year_list:
                try:
                    ...
                    table = data_extraction.create_scenario_selection_table_with_numbers(project_name=project_name)
                    ref = db.read_dataframe(
                        OperationTable.ResultRefYear, column_names=["Grid", "Load", "ID_Scenario"]
                    ).rename(columns={"Grid": "Grid demand consumer", "Load": "Load consumer"})
                    opt = db.read_dataframe(
                        OperationTable.ResultOptYear, column_names=["Grid", "Load"]
                    ).rename(columns={"Grid": "Grid demand prosumager", "Load": "Load prosumager"})
                    df = pd.merge(left=table, right=pd.concat([ref, opt], axis=1), on="ID_Scenario")
                    df["Grid demand consumer (GWh)"] = df["Grid demand consumer"] * df["number"] / 1_000_000_000
                    df["Grid demand prosumager (GWh)"] = df["Grid demand prosumager"] * df["number"] / 1_000_000_000
                    df["Load consumer (GWh)"] = df["Load consumer"] * df["number"] / 1_000_000_000
                    df["Load prosumager (GWh)"] = df["Load prosumager"] * df["number"] / 1_000_000_000
                    df["Grid demand difference (GWh)"] = df["Grid demand prosumager (GWh)"] - df["Grid demand consumer (GWh)"]
                    df["Load difference (GWh)"] = df["Load prosumager (GWh)"] - df["Load consumer (GWh)"]
                    df_sum = df.sum(axis=0)
                    df_sum["Grid demand difference (%)"] = df_sum["Grid demand difference (GWh)"] / df_sum["Grid demand consumer (GWh)"] * 100
                    df_sum["Load difference (%)"] = df_sum["Load difference (GWh)"] / df_sum["Load consumer (GWh)"] * 100
                    new_df = pd.DataFrame(df_sum[[
                        "Grid demand difference (%)",
                        "Load difference (%)",
                        ]]).T
                    new_df[["year", "country"]] = int(year), country
                    big_df = pd.concat([big_df, new_df], axis=0)


                except:
                    print(f"{country} {year} can not access consumption data")
        number_years = len(year_list)
        plot_df = big_df.melt(id_vars=["year", "country"], value_name="% increase")
        fig = px.bar(
            data_frame=plot_df,
            x="country",
            y="% increase",
        ...
        return html.Div(dcc.Graph(figure=fig), id=ids.ELECTRICITY_CONSUMPTION_DIFFERENCE)

    return html.Div(id=ids.ELECTRICITY_CONSUMPTION_DIFFERENCE)
...
def cost_reduction_box_plot_all_countries(app: Dash):
    """ calculate the cost reduction through prosumaging by comparing ref to opt costs"""
    big_df = pd.DataFrame()
    for country in main.COUNTRIES:
        for year in main.YEARS:
            try:
                table = data_extraction.create_scenario_selection_table_with_numbers(
                    project_name=f"{main.PROJECT_PREFIX}_{country}_{year}"
                )
                db = DB(config=Config(project_name=f"{main.PROJECT_PREFIX}_{country}_{year}"))
                cost_opt = db.read_dataframe(
                    OperationTable.ResultOptYear, column_names=["ID_Scenario", "TotalCost"]
                ).rename(columns={"TotalCost": "Prosumager cost (€)"})
                cost_opt["Prosumager cost (€)"] = cost_opt["Prosumager cost (€)"] / 100
                cost_ref = db.read_dataframe(
                    OperationTable.ResultRefYear, column_names=["TotalCost"]
                    ).rename(columns={"TotalCost": "Consumer cost (€)"}) / 100
                df = pd.merge(left=table, right=pd.concat([cost_ref, cost_opt], axis=1), on="ID_Scenario")

                df["Prosumager savings (€)"] = (df["Consumer cost (€)"] - df["Prosumager cost (€)"])
                df["Prosumager savings (%)"] = (df["Prosumager savings (€)"] / (df["Consumer cost (€)"])) * 100
                df[["year", "country"]] = year, country
                big_df = pd.concat([big_df, df], axis=0)
            except:
                print(f"Cost data is not available for {country} {year}")

    names = ["ID_Battery", "ID_Boiler", "ID_HotWaterTank", "ID_SpaceHeatingTank", "ID_PV"]
    dash_ids = [f"{name}_cost" for name in names]  # find these in dropdown.py
    callback_inputs = [Input(dash_id, "value") for dash_id in dash_ids] + [Input(ids.COST_DROP_DOWN, "value")]
    @app.callback(Output(ids.COST_REDUCTION, "children"),
                  callback_inputs)
    def update_figure(battery_id, boiler_id, dhw_id, buffer_id, pv_id, cost_dropdown):
        plot_df = big_df.query(
           f"ID_Battery in {battery_id} and "
           f"ID_Boiler in {boiler_id} and "
           f"ID_HotWaterTank in {dhw_id} and "
           f"ID_SpaceHeatingTank in {buffer_id} and "
           f"ID_PV in {pv_id}"
        )
        ...
        return html.Div(dcc.Graph(figure=fig), id=ids.COST_REDUCTION)

    return html.Div(id=ids.COST_REDUCTION)



if __name__ == "__main__":
    EU27_electricity_consumption_difference_country_level(1)
