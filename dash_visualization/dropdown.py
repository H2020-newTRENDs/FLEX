...
def generation_shift_drop_down(app: Dash) -> html.Div:
    return html.Div(
        children=[
            html.H6("Country"),
            dcc.Dropdown(
                id=ids.COUNTRY_DROP_DOWN_GENERATION_SHIFT,
                options=[{"label": country, "value": country} for country in main.COUNTRIES],
                value="AUT",
                multi=False,
            ),
...
def create_dropdown_options_for_cost_reduction_plot(app: Dash) -> list:
    ...
    drop_down_menus = []
    for name in names:
        values = df[name].unique().tolist()
        identity = f"{name}_cost"
        drop_down_menus.append(
            html.Div(
                children=[
                    html.H6(f"{name}"),
                    dcc.Dropdown(
...
def dropdown_options_for_cost(app: Dash) -> html.Div:
    return html.Div(
        children=[
            html.H6("Cost"),
            dcc.Dropdown(
                id=ids.COST_DROP_DOWN,
                options=["Prosumager savings (%)", "Prosumager savings (€)", "Consumer cost (€)", "Prosumager cost (€)"],
                value="Prosumager savings (%)",
                multi=False,
            ),
        ]
    )
...
def dropdown_generation_slice_year_options():
    return html.Div(children=[
        html.H6("Years"),
        dcc.Dropdown(
            id=ids.SLICE_GENERATION_SHIFT_YEAR_DROP_DOWN,
            options=main.YEARS,
            value=main.YEARS[0],
            multi=False,
        )
