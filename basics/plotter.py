...
class Plotter:
    ...
    def setup_fig_params(self):
        self.fig_size = (12, 6)
        self.fig_axe_area = (0.1, 0.1, 0.8, 0.7)
        self.fig_format = "PNG"
        self.fig_dpi = 200
        self.line_width = 1.5
        self.line_style = '-'
    ...
    def save_fig(self, fig, fig_name):
        fig.savefig(os.path.join(self.fig_folder, fig_name + ".png"), dpi=self.fig_dpi, format=self.fig_format)
        plt.close(fig)

    def get_figure_plate(self, x_label: str = "X-axis", y_label: str = "Y-axis", x_lim=None, y_lim=None):
        ...
        if x_lim is not None: ax.set_ylim(x_lim)
    ...
    def add_legend(self, figure, ax, num_items: int):
        figure.legend(fontsize=self.legend_fontsize, bbox_to_anchor=self.top_bbox_to_anchor,
                      bbox_transform=ax.transAxes, loc='lower left', ncol=self.legend_ncol(num_items),
                      borderaxespad=0, mode='expand', frameon=True)

    def step_figure(self, values_dict: 'Dict[str, Sequence]', fig_name: str,
                    x_label: str = "X-axis", y_label: str = "Y-axis", x_lim=None, y_lim=None):
        figure, ax = self.get_figure_plate(x_label, y_label, x_lim, y_lim)
    ...
    def bar_plot(self, values_dict: 'Dict[str, np.array]', fig_name: str,
                 x_label: str = "X-axis", y_label: str = "Y-axis", x_lim=None, y_lim=None):
        figure, ax = self.get_figure_plate(x_label, y_label, x_lim, y_lim)
        x = [i + 1 for i in range(0, len(list(values_dict.values())[0]))]
        ...
        for key, values in values_dict.items():
            if values.mean() > 0:
                ax.bar(x, values, bottom=bottom_positive, label=key,
                       color=[Color.__dict__[key].value for i in range(0, len(x))])
                bottom_positive += values
            else:
                ax.bar(x, values, bottom=bottom_negative, label=key,
                       color=[Color.__dict__[key].value for i in range(0, len(x))])
